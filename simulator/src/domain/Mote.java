...
import java.util.*;
// import domain.PacketLinkCombination;
...
public class Mote extends Node {
    ...
    // private HashMap<Integer, List<String>> packetGeneratedPacketLinkStore;
    HashMap<Packet, PacketLinkCombination> trackQueue;
    ...
    public boolean isLeafMote() {
        return leafMote;
    }

    public void setLeafMote(boolean leafMote) {
    ...
    public Mote(int id, double batteryCapacity, int load, int maxQueueSize, int threshold, Position position,
                boolean leafMote, ChannelConfiguration channelconfig) {
        super(id, position);
        this.batteryCapacity = batteryCapacity;
        this.batteryRemaining = batteryCapacity;
        this.load = load;
        ...
        this.setLeafMote(leafMote);
        
        this.channelConfig = channelconfig;
        this.activeChannelFrequency = 866.10; // standard mid of frequency band
        trackQueue = new HashMap<Packet, PacketLinkCombination>();
    }
    public Mote(int id, double batteryCapacity, int load, int maxQueueSize, int threshold, Position position,
                boolean leafMote) {
        super(id, position);
        ...
        this.channelConfig = new ChannelConfiguration("BASIC");
        this.activeChannelFrequency = 866.10; // standard mid of frequency band
        trackQueue = new HashMap<Packet, PacketLinkCombination>();
    }
...
//			}
            ...
            // else the distribution is handled like weights
                ...
                for (Link link : possibleLinks) {
                    ...
                    if (countDistribution >= rand) {
                        // store info before trying to sent over
                        // create packetlinkcombination and store it 
                        PacketLinkCombination pkl = new PacketLinkCombination(packet, link, runInfo.getRunNumber());
                        packet.getDestination().storeGeneratedPacketLinkStore(pkl, runInfo);
                        trackQueue.put(packet, pkl);
                        sendPacketOver(link, packet, runInfo);
                        break;
                    }
                }
        ...
        while (!packetQueue.isEmpty()) {
            ...
            if (runInfo.getRunNumber() - packet.getStartingRun() >= keepAliveTime) {
                // if a packet is to old, we will add it to the packetlink store of lost packages
                // determine link packet was to be send over
               
                PacketLinkCombination pkl = trackQueue.get(packet);
                packet.getDestination().storeLostQueueLinkStore(pkl, runInfo);
        ...
        // calculate the rssi for the links we used to send packets to
        // initial calculation, currently static as only depending on distance
        // use runinfo to calculate only in first cycle
